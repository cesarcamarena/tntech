...
//{
   ...
   const char* fileName2 = "sphere_trs.txt";
...
BasicObject* Picture::readObject(const char* file_name)
{
   ...
   while (!rf->eof())
   {
      ...
      else if (token->compare(vt) == 0)
      {
         ...
         obj->getFace(count)->setSCoords(s1, s2, s3);
         obj->getFace(count)->setTCoords(t1, t2, t3);
...
InstanceObject* Picture::buildInstanceObject(const char* file_name, BasicObject* obj)
{
   ...
   delete line;
   Texture* texture = new Texture(t_file, t_width, t_height);
