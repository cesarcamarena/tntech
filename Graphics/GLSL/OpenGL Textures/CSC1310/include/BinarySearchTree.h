...
class BinarySearchTree
{
   private:
      ...
      TreeNode<T>* removeItem(TreeNode<T>* tNode, CSC2110::String* sk);
      ...
      int (*compare_keys) (CSC2110::String* key, T* item);
	  
	  void checkInorderSuccessor(TreeNode<T>* tNode, CSC2110::String* sk, CSC2110::ListArray<T>* items);

   public:
      BinarySearchTree(int (*comp_items) (T* item_1, T* item_2), int (*comp_keys) (CSC2110::String* key, T* item));
      ~BinarySearchTree();

      bool isEmpty();
      void makeEmpty();

      T* retrieve(CSC2110::String* search_key);
	  CSC2110::ListArray<T>* retrieveDuplicates(CSC2110::String* search_key);
      void insert(T* item);
      void remove(CSC2110::String* search_key);
...
template < class T >
void BinarySearchTree<T>::checkInorderSuccessor(TreeNode<T>* tNode, CSC2110::String* sk, CSC2110::ListArray<T>* items)
{
...
template < class T >
CSC2110::ListArray<T>* BinarySearchTree<T>::retrieveDuplicates(CSC2110::String* sk)
{
	CSC2110::ListArray<T>* items = new CSC2110::ListArray<T>();
...
template < class T >
T* BinarySearchTree<T>::retrieve(CSC2110::String* sk)
{
   TreeNode<T>* tNode = getRootNode();
...
template < class T >
void BinarySearchTree<T>::remove(CSC2110::String* sk)
{
   root = removeItem(root, sk);
}

template < class T >
TreeNode<T>* BinarySearchTree<T>::removeItem(TreeNode<T>* tNode, CSC2110::String* sk)
{
   if (tNode == NULL)
   { 
...
template < class T >
BinarySearchTree<T>::BinarySearchTree(int (*comp_items) (T* item_1, T* item_2), int (*comp_keys) (CSC2110::String* key, T* item))
{
   root = NULL;
   sze = 0;
