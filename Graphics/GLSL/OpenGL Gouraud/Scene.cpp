...
void Scene::render(int w, int h)
{
   ...
   glLoadIdentity();
   float ratio = ((double) h)/w;  //height/width (can use fovx if ratio is w/h)
   ...
   GLfloat position0[] = {lightLoc->getX(), lightLoc->getY(), lightLoc->getZ(), 1.0};  //xyzw         
   GLfloat specular0[] = {1.0, 1.0, 1.0, 1.0};

   //use a global ambient light source that is a very dark red (using both light and material ambient components)
   GLfloat ambientLight[] = {ambient->getRed(), ambient->getGreen(), ambient->getBlue(), 1.0};  //a dark grey
