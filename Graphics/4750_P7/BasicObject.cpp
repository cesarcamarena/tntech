...
void BasicObject::render(Pixel* px, Matrix* sceneTransform, Matrix* instanceTransform, Vertex* eye, Light* light, Color* diffuse, Color* amb, double atten, double shine, Matrix* zbuffer, unsigned char* colors, int t_width, int t_height)
{
   ListArrayIterator<Face>* faceIter = faces->iterator();
   ...
   {
      Face* face = faceIter->next();
      face->render(px, sceneTransform, instanceTransform, eye, light, diffuse, amb, atten, shine, zbuffer, colors, t_width, t_height);
   }
