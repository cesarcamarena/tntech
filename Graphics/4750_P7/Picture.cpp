...
//{
   ...
   const char* fileName1 = "cube_texture.txt";
   BasicObject* sphere = readObject(fileName1);
   //delete[] fileName;

   //const char* fileName2 = "trs.txt";
   const char* fileName2 = "cube_trs.txt";
   ...
   /*Texture* ttu = new Texture("TTU.raw", 128, 128);
   unsigned char* colors = sphereInstance->readTexture(ttu);
   cout << colors[0] << endl;
   
	const char* fileName6 = "MoonMap.raw";
	Texture* moon = new Texture(fileName6, 800, 400);
	unsigned char* colors = sphereInstance->readTexture(moon);
   */
...
BasicObject* Picture::readObject(const char* file_name)
{
   ...
   while (!rf->eof())
   {
	  ...
	  else if(token->compare(vt) == 0)
	  {
		  ...
		  obj->getFace(count)->addSTexel(s1);
		  obj->getFace(count)->addSTexel(s2);
		  obj->getFace(count)->addSTexel(s3);
		  obj->getFace(count)->addTTexel(t1);
		  obj->getFace(count)->addTTexel(t2);
		  obj->getFace(count)->addTTexel(t3);
...
InstanceObject* Picture::buildInstanceObject(const char* file_name, BasicObject* obj)
{
   ...
   delete line;
   line = rf->readLine();
   char* textureFile = line->get_c_str();
   delete line;
   line = rf->readLine();
   int t_width = line->a_to_i();
   delete line;
   line = rf->readLine();
   int t_height = line->a_to_i();
   delete line;
   Texture* texture = new Texture(textureFile, t_width, t_height);
   io->setTexture(texture);
