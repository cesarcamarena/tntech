...
void Face::render(Pixel* px, Matrix* sceneTransform, Matrix* instanceTransform, Vertex* eye, Light* light, Color* diffuse, Color* ambient, double atten, double shine, Matrix* zbuffer, unsigned char* colors, int t_width,int t_height)
{
   ...
   {
      drawTriangle(v1, v2, v3, zbuffer, px, colors, t_width, t_height, lightLoc);
   }
   delete n;
   //delete lightLoc;
   //delete colors;
...
void Face::drawTriangle(Vertex* first, Vertex* second, Vertex* third, Matrix* zbuffer, Pixel* px, unsigned char* colors, int w, int h, Vertex* lightLoc)
{
   ...
   double over_h = (1/first->getH()) + beta_start*(1/second->getH() - 1/first->getH()) + gamma_start*(1/third->getH() - 1/first->getH());
   double s_over_h = (s_texels[0]/first->getH()) + beta_start*(s_texels[1]/second->getH() - s_texels[0]/first->getH()) + gamma_start*(s_texels[2]/third->getH() - s_texels[0]/first->getH());
   double t_over_h = (t_texels[0]/first->getH()) + beta_start*(t_texels[1]/second->getH() - t_texels[0]/first->getH()) + gamma_start*(t_texels[2]/third->getH() - t_texels[0]/first->getH());
   
   double s_prime = s_over_h/over_h;
   double t_prime = t_over_h/over_h;
   
   double stexel = ((int)(s_prime * w))%w;
   double ttexel = ((int)(t_prime * h))%h;
      ...
      for (int y = (int) ymin; y <= (int) ymax; y++)
      {
         ...
         if (beta <= 1 && beta >= 0 && gamma <= 1 && gamma >= 0 && beta + gamma <= 1 )  
         {
            if (z > zbuffer->getElement(y + 1, x + 1) && z < 1)
            {
               ...
               //Color color(r, g, b);
			   over_h = (1/first->getH()) + beta*(1/second->getH() - 1/first->getH()) + gamma*(1/third->getH() - 1/first->getH());
			   s_over_h = (s_texels[0]/first->getH()) + beta*(s_texels[1]/second->getH() - s_texels[0]/first->getH()) + gamma*(s_texels[2]/third->getH() - s_texels[0]/first->getH());
			   t_over_h = (t_texels[0]/first->getH()) + beta*(t_texels[1]/second->getH() - t_texels[0]/first->getH()) + gamma*(t_texels[2]/third->getH() - t_texels[0]/first->getH());
   
			   s_prime = s_over_h/over_h;
			   t_prime = t_over_h/over_h;
   
			   stexel = ((int)(s_prime * w))%w;
			   ttexel = ((int)(t_prime * h))%h;
   
			   ttexel = h - ttexel - 1;
			   
			   int index = (ttexel*w + stexel) * 3;
			   
			   Vector* l_tmp = lightLoc->subtract(first);
			   Vector* l = l_tmp->normalize();
			  
			   
			   Vector* n = first->getNormal();
			   ...
			   double r_tex = colors[index + 0]/255.0*r; 
			   double g_tex = colors[index + 1]/255.0*g;
			   double b_tex = colors[index + 2]/255.0*b;
...
void Face::addSTexel(double s)
{
	s_texels[s_sz] = s;
	s_sz++;
}

void Face::addTTexel(double t)
{
	t_texels[t_sz] = t;
	t_sz++;
}