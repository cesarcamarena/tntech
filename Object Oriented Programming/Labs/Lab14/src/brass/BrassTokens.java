...
class BrassTokens
{
	...
	private List<BrassToken> all_brass_tokens;  //for save/load
	
	public int getTechLevel(int industry_id)
	{
		List<BrassToken> brass_industry_specific_tokens = brass_tokens.get(industry_id - 1);
		assert brass_industry_specific_tokens.size() > 0 : "No industry tokens remaining.";
	...
	{
		List<BrassToken> brass_industry_specific_tokens = brass_tokens.get(token_id - 1);
		assert brass_industry_specific_tokens.size() > 0 : "No token to discard.";
		brass_industry_specific_tokens.remove(0); 
	}
	
	public boolean isCoalRequired(int industry_id)
	{
		List<BrassToken> brass_industry_specific_tokens = brass_tokens.get(industry_id - 1);
		BrassToken brass_token = brass_industry_specific_tokens.get(0); 
		return brass_token.isCoalRequired();
	}
	...
	public boolean isIronRequired(int industry_id)
	{
		List<BrassToken> brass_industry_specific_tokens = brass_tokens.get(industry_id - 1);
		BrassToken brass_token = brass_industry_specific_tokens.get(0); 
		return brass_token.isIronRequired();
	}
	...
	*/
	...
	{
		return all_brass_tokens.get(token_id - 1);
	}
	
	org.jdom2.Element writeXML()
   {
		org.jdom2.Element all_tokens_element = new org.jdom2.Element("tokens");
	  
		//java.util.Set<BrassIndustryEnum> industry_enums = java.util.EnumSet.allOf(BrassIndustryEnum.class);
		for (BrassIndustryEnum industry_enum : BrassIndustryEnum.values())
		{
			int industry_index = industry_enum.getValue();
			List<BrassToken> write_tokens = brass_tokens.get(industry_index-1);
	//make sure the order is correct
	//System.out.println(industry_enum.getName());
			org.jdom2.Element write_tokens_element = new org.jdom2.Element(industry_enum.getName());
			for (BrassToken write_token : write_tokens)
			{
				org.jdom2.Element write_token_element = write_token.writeXML();
				write_tokens_element.addContent(write_token_element);
			}
			all_tokens_element.addContent(write_tokens_element);
		}

		return all_tokens_element;
   }
   
	void readXML(org.jdom2.Element all_tokens_element)
	{	
		//java.util.Set<BrassIndustryEnum> industry_enums = java.util.EnumSet.allOf(BrassIndustryEnum.class);
		for (BrassIndustryEnum industry_enum : BrassIndustryEnum.values())
		{
			int industry_index = industry_enum.getValue();
			org.jdom2.Element all_token_element = all_tokens_element.getChild(industry_enum.getName());
			List<org.jdom2.Element> token_list_element = all_token_element.getChildren("token");
			brass_tokens.get(industry_index-1).clear();
		  
			for (org.jdom2.Element token_element : token_list_element)
			{
				org.jdom2.Element token_id_element = token_element.getChild("token_id");
				int token_id = Integer.parseInt(token_id_element.getText());
			
				BrassToken brass_token = all_brass_tokens.get(token_id - 1);
				brass_tokens.get(industry_index-1).add(brass_token);
				PixelPoint player_token_location = player_industry_locations.get(industry_index-1);
				brass_token.placePlayerToken(player_token_location.getX(), player_token_location.getY());
