...
class BrassPlayer
{
	...
	{
		return brass_hand.isFirstAction();
	}
	...
	{
		//already showing rail images
		if (!player_canal_img.isVisible()) return;
		
		player_canal_img.hideImage();
		
		//show the rail image in the same location as the canal image
	...
	{
		//already showing canal images
		if (!player_rail_img.isVisible()) return;
		
		player_rail_img.hideImage();
		
		int canal_x = player_rail_img.getXCenter();
		int canal_y = player_rail_img.getYCenter();
		
		player_canal_img.showImage(canal_x, canal_y);
	}
	...
	BrassToken getBrassToken(int token_id)
	{
		return brass_tokens.getBrassToken(token_id);
	}
	
	org.jdom2.Element writeXML()
   {
      org.jdom2.Element player_element = new org.jdom2.Element("player");
	  
	  org.jdom2.Element player_id_element = new org.jdom2.Element("id");
	  player_id_element.setText("" + player_id);
	  player_element.addContent(player_id_element);
	  
	  org.jdom2.Element money_element = new org.jdom2.Element("money");
	  money_element.setText("" + money);
	  player_element.addContent(money_element);
	  
	  org.jdom2.Element amount_spent_element = new org.jdom2.Element("spent");
	  amount_spent_element.setText("" + amount_spent);
	  player_element.addContent(amount_spent_element);
	  
	  org.jdom2.Element income_element = new org.jdom2.Element("income");
	  income_element.setText("" + income_index);
	  player_element.addContent(income_element);
	  
	  org.jdom2.Element victory_element = new org.jdom2.Element("victory");
	  victory_element.setText("" + victory_index);
	  player_element.addContent(victory_element);
	  
	  org.jdom2.Element hand_element = brass_hand.writeXML();
	  player_element.addContent(hand_element);
	  
	  org.jdom2.Element tokens_element = brass_tokens.writeXML();
	  player_element.addContent(tokens_element);
	  
      return player_element;
   }
   
	void readXML(org.jdom2.Element player_element, BrassDeck brass_deck, BrassTrack brass_track)
	{	
		org.jdom2.Element money_element = player_element.getChild("money");
		money = Integer.parseInt(money_element.getText());

		org.jdom2.Element amount_spent_element = player_element.getChild("spent");
		amount_spent = Integer.parseInt(amount_spent_element.getText());
	  
		org.jdom2.Element income_element = player_element.getChild("income");
		income_index = Integer.parseInt(income_element.getText());
	  
		org.jdom2.Element victory_element = player_element.getChild("victory");
		victory_index = Integer.parseInt(victory_element.getText());
		
		updateTrackMarkers(brass_track);
	  
		org.jdom2.Element tokens_element = player_element.getChild("tokens");
		brass_tokens.readXML(tokens_element);
		
		org.jdom2.Element hand_element = player_element.getChild("hand");
		brass_hand.readXML(hand_element, brass_deck);
		brass_hand.showHand();
	}
}
