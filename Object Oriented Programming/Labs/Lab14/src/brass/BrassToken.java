...
class BrassToken
{
	...
	{
		assert (cubes > 0) : "No cubes to remove.";
		cubes--;
		
		if (cubes == 0)
		{
			flipped = true;
			brass_players.increaseIncomeIndex(player_id, income, brass_track);
		}
	}
	
	public boolean isIronRequired()
	{
		return iron_requirement;
	}
	
	public boolean isCoalRequired()
	{
		return coal_requirement;
	}
	...
	void readXML(org.jdom2.Element token_element)
	{	
		org.jdom2.Element token_flipped_element = token_element.getChild("flipped");
		int flip = Integer.parseInt(token_flipped_element.getText());
		flipped = false;
		if (flip == 1) flipped = true;
		
		org.jdom2.Element token_cubes_element = token_element.getChild("cubes");
		if (token_cubes_element != null)
		{
			int num_cubes = Integer.parseInt(token_cubes_element.getText());
			cubes = num_cubes;
		}
	}
   ...
   {
      org.jdom2.Element token_element = new org.jdom2.Element("token");

      org.jdom2.Element token_id_element = new org.jdom2.Element("token_id");
      token_id_element.setText("" + token_id);
      token_element.addContent(token_id_element);
	  
	  org.jdom2.Element player_id_element = new org.jdom2.Element("player_id");
      player_id_element.setText("" + player_id);
      token_element.addContent(player_id_element);
	  
	  org.jdom2.Element cubes_element = new org.jdom2.Element("cubes");
      cubes_element.setText("" + cubes);
      token_element.addContent(cubes_element);
	  
	  org.jdom2.Element flipped_element = new org.jdom2.Element("flipped");
	  if (!flipped)
	  {
		  flipped_element.setText("" + 0);
	  }
	  else
	  {
		  flipped_element.setText("" + 1);
	  }
      token_element.addContent(flipped_element);
