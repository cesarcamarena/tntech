...
class BrassHand
{
	...
	{
		return (card_index <= getNumCards()) && (sel_card_1 != card_index);
	}
	
	public void cancelDoubleCardSelection()
	{
		BrassCard brass_card;
		
		brass_card = brass_hand.get(sel_card_1 - 1);
		...
		//if two cards had already been selected (most likely) cancel them both
		if (sel_card_2 > 0)
		{
			brass_card = brass_hand.get(sel_card_2 - 1);
			//remember that the first set of small card centers is the small card reverse side
   ...
   {
      org.jdom2.Element hand_element = new org.jdom2.Element("hand");
	  
	  for (BrassCard brass_card : brass_hand)
	  {
		  org.jdom2.Element card_element = brass_card.writeXML();
		  hand_element.addContent(card_element);
	  }

      return hand_element;
   }
   
	void readXML(org.jdom2.Element hand_element, BrassDeck brass_deck)
	{	
		brass_hand.clear();
		sel_card_1 = 0;
		sel_card_2 = 0;
		List<org.jdom2.Element> cards_element = hand_element.getChildren("card");
	  
		for (org.jdom2.Element card_element : cards_element)
		{
			org.jdom2.Element id_element = card_element.getChild("id");
			int card_id = Integer.parseInt(id_element.getText());
			BrassCard brass_card = brass_deck.getUnshuffledCard(card_id);
			brass_card.hideBigCard();
			brass_hand.add(brass_card);
		}
	}
