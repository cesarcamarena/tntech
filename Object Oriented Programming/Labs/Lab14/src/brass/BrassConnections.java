...
abstract class BrassConnections
{
	...
	public void buildLink(int link_id, int player_id)
	{
		...
		BrassConnection brass_connection = brass_connections.get(link_id - 1);
		DrawImage brass_connection_img = new DrawImage(brass_connection_images.get(player_id - 1), "Brass Connection " + link_id, connection_dimension.getWidth(), connection_dimension.getHeight());
   ...
   {
      org.jdom2.Element links_element = new org.jdom2.Element("links");
	  
	  for (BrassConnection brass_connection : brass_connections)
	  {
		  org.jdom2.Element link_element = brass_connection.writeXML();
		  links_element.addContent(link_element);
	  }

      return links_element;
   }
   
	void readXML(org.jdom2.Element links_element)
	{	
		removeLinks();
		List<org.jdom2.Element> links_list_element = links_element.getChildren("link");
	  
		for (org.jdom2.Element link_element : links_list_element)
		{
			org.jdom2.Element link_id_element = link_element.getChild("link_id");
			int link_id = Integer.parseInt(link_id_element.getText());
			
			org.jdom2.Element player_id_element = link_element.getChild("player_id");
			int player_id = Integer.parseInt(player_id_element.getText());
		
			//BrassConnection brass_connection = brass_connections.get(link_id - 1);
			if (player_id > 0)
			{
				buildLink(link_id, player_id);
			}
		}
	}
}
