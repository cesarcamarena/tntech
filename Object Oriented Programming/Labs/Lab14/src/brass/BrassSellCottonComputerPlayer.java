...
class BrassSellCottonComputerPlayer implements BrassComputerPlayer
{
	...
	public BrassComputerPlayerAction getBrassMove()
	{
		...
		if (!can_sell_cotton)
		{
			getAllInfoForLinkToSellCottonAction(computer_move, computer_player_id, 2);
			if (computer_move.isActionSelected()) return computer_move;
		}
	...
	System.out.println("Discard!");
		...
		/*	
		int num_coal_cubes_on_board = brass_game.getNumCubesOnBoard(1);
		int num_player_coal_cubes_on_board = brass_game.getNumPlayerCubesOnBoard(1, computer_player_id);
		//also count your own coal mines
System.out.println("coal cubes: " + num_coal_cubes_on_board);
		if (num_coal_cubes_on_board < 3 && num_player_coal_cubes_on_board == 0)
		{
			getAllInfoForIndustrySpecificBuildAction(computer_move, computer_player_id, 1);
			if (computer_move.isActionSelected()) return computer_move;
		}
		
		//count the number of iron cubes
		int num_iron_cubes_on_board = brass_game.getNumCubesOnBoard(3);
		int num_player_iron_cubes_on_board = brass_game.getNumPlayerCubesOnBoard(3, computer_player_id);
System.out.println("iron cubes: " + num_iron_cubes_on_board);
		if (num_iron_cubes_on_board < 5 && num_player_iron_cubes_on_board == 0)
		{
			getAllInfoForIndustrySpecificBuildAction(computer_move, computer_player_id, 3);
			if (computer_move.isActionSelected()) return computer_move;
		}
		
		if (num_iron_cubes_on_board >= 2)
		{
			getAllInfoForTechUpgradeAction(computer_move, computer_player_id);
			if (computer_move.isActionSelected()) return computer_move;
		}
...
System.out.println("Regular Link");					
		...
		private void getAllInfoForTechUpgradeAction(BrassComputerPlayerAction computer_move, int computer_player_id)
	{
	...
	private void getAllInfoForLinkAction(BrassComputerPlayerAction computer_move, int computer_player_id)
	{
		int num_connections = brass_game.getNumConnections();
		for (int i = 1; i <= num_connections; i++)
		{
			boolean can_build_link = BrassUtilities.canBuildLink(brass_game, i, computer_player_id);
			if (can_build_link)
			{
				computer_move.selectLinkAction(getCardForNonBuildAction(computer_player_id), i);
System.out.println("Regular Link");					
