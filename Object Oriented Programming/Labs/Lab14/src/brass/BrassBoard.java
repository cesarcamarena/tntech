...
class BrassBoard
{
	...
	public void activateRails()
	{
		brass_canals = brass_connections;
...
System.out.println("industries: " + industry_victory);
	...
	public BrassBoard(BrassXML brass_xml)  
	{
		...
		brass_connections = brass_canals;
   ...
   {
		org.jdom2.Element board_element = new org.jdom2.Element("board");
		
		org.jdom2.Element cotton_demand_element = brass_cotton_demand_track.writeXML();
		board_element.addContent(cotton_demand_element);
		
		org.jdom2.Element coal_demand_element = brass_coal_demand_track.writeXML("coal_demand_track");
		board_element.addContent(coal_demand_element);
		
		org.jdom2.Element iron_demand_element = brass_iron_demand_track.writeXML("iron_demand_track");
		board_element.addContent(iron_demand_element);
		
		org.jdom2.Element cities_element = brass_cities.writeXML();
		board_element.addContent(cities_element);
		
		org.jdom2.Element connections_element = brass_connections.writeXML();
		board_element.addContent(connections_element);

		return board_element;
   }
   
	void readXML(org.jdom2.Element board_element, boolean brass_phase, BrassPlayers brass_players)
	{	
		org.jdom2.Element cotton_demand_element = board_element.getChild("cotton_demand");
		brass_cotton_demand_track.readXML(cotton_demand_element);
		
		org.jdom2.Element coal_demand_element = board_element.getChild("coal_demand_track");
		brass_coal_demand_track.readXML(coal_demand_element);
		
		org.jdom2.Element iron_demand_element = board_element.getChild("iron_demand_track");
		brass_iron_demand_track.readXML(iron_demand_element);
		
		org.jdom2.Element cities_element = board_element.getChild("cities");
		brass_cities.readXML(cities_element, brass_players);
		
		if (!brass_phase && (brass_connections == brass_rails))
		{
			brass_connections = brass_canals;
		}
		
		org.jdom2.Element connections_element = board_element.getChild("links");
		brass_connections.readXML(connections_element);
