...
class BrassCity implements util.Execute<BrassIndustry>
{
   ...
   {
      org.jdom2.Element city_element = new org.jdom2.Element("city");
	  
	  org.jdom2.Element city_id_element = new org.jdom2.Element("city_id");
      city_id_element.setText("" + brass_city_id);
      city_element.addContent(city_id_element);
	  
	  for (BrassIndustry brass_industry : brass_industries)
	  {
		  org.jdom2.Element industry_element = brass_industry.writeXML();
		  city_element.addContent(industry_element);
	  }

      return city_element;
   }
   
	void readXML(org.jdom2.Element city_element, BrassPlayers brass_players)
	{	
		List<org.jdom2.Element> industries_list_element = city_element.getChildren("industry");
	  
		int count = 1;
		for (org.jdom2.Element industry_element : industries_list_element)
		{
			BrassIndustry brass_industry = brass_industries.get(count - 1);
			brass_industry.readXML(industry_element, brass_players);
			count++;
		}
	}
}
