...
class BrassBuildActionState implements BrassState
{
	...
	public void mouseClicked(int x_click, int y_click)
	{
		...
		{
			//coal_city_id should not be -1 if we make it here
			/*
			int coal_city_id = brass_game.canMoveCoal(city_id, industry_id, player_id);
			assert coal_city_id >= 0 : "Coal cannot be moved.";
			if (coal_city_id > 0)
			{
				brass_game.moveCoal(coal_city_id, player_id);
			}
			
			int iron_city_id = brass_game.canMoveIron(industry_id, player_id);
			assert iron_city_id >= 0 : "Iron cannot be obtained.";
			if (iron_city_id > 0)
			{
				brass_game.moveIron(iron_city_id, player_id);
			}
			*/
