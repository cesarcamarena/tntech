...
class BrassDeck
{
	...
	private ArrayList<BrassCard> shuffle()
	{
		...
		while (perm.hasNext())
		{
			int randomPos = perm.next();
			
			System.out.println(randomPos);
			
			BrassCard bc = brass_deck.get(randomPos-1);
		
			shuffled_deck.add(bc);
		}

	System.out.println("NUM CARDS: " + shuffled_deck.size());
	...
	{
		//use the deck returned from the shuffle method
		//shuffled_deck = new ArrayList<BrassCard>();
		int sz = shuffled_deck.size();
		BrassCard bc = shuffled_deck.remove(sz-1);
