...
class BrassBoard
{
	...
	{
		//DO THIS
		//check to see if the player is present in the city (method getNumTokensInCity in BrassCities, will return a number > 0 if a player is present)
		int number_of_tokens = brass_cities.getNumTokensInCity(city_id, player_id);

		//check to see if the player has a link into the city (method doesPlayerHaveLinkIntoCity in BrassConnections)
		boolean player_has_link_into_city = brass_connections.doesPlayerHaveLinkIntoCity(city_id, player_id);

		//return a statement checking if the player is present OR the player has a link in the city
		if (number_of_tokens > 0 || player_has_link_into_city == true)
		{
			return true;
		}
		return false;
	...
	public boolean isLinkConnectedToPlayerNetwork(int link_id, int player_id)
	{
		...
		/*boolean city_1 = brass_connections.doesPlayerHaveLinkIntoCity(city_1_id, player_id);

		boolean city_2 = brass_connections.doesPlayerHaveLinkIntoCity(city_2_id, player_id);*/
		boolean city_1 = isCityConnectedToPlayerNetwork(city_1_id, player_id);
		boolean city_2 = isCityConnectedToPlayerNetwork(city_2_id, player_id);
		System.out.println("LINK STATUS BLOCK");
		System.out.println(city_1_id);
		System.out.println(city_2_id);
		System.out.println(city_1);
		System.out.println(city_2);
		if (city_1 == true || city_2 == true)
		{
			System.out.println("LINK CONNECTION AFFIRMED.");
			return true;
		}
		System.out.println("LINK CONNECTION NEGATIVE.");
		return false;
	}
