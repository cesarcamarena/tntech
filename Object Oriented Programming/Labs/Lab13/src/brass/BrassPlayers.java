...
class BrassPlayers
{
	...
	public boolean isFirstAction()
	{
		...
		return brass_player.isFirstAction();
	}
	...
	public void changePhaseBack()
	{
		Iterator<BrassPlayer> brass_player_iter = brass_players.iterator();
		while(brass_player_iter.hasNext())
		{
			BrassPlayer brass_player = brass_player_iter.next();
			brass_player.changePhaseBack();
		}
	}
   ...
   BrassToken getBrassToken(int player_id, int token_id)
	{
		BrassPlayer brass_player = brass_players.get(player_id - 1);
		return brass_player.getBrassToken(token_id);
	}
	
	org.jdom2.Element writeXML()
   {
      org.jdom2.Element players_element = new org.jdom2.Element("players");
	  
	  org.jdom2.Element active_element = new org.jdom2.Element("active");
	  active_element.setText("" + active_player_id);
	  players_element.addContent(active_element);
	  
	  org.jdom2.Element taken_turn_element = new org.jdom2.Element("turn");
	  taken_turn_element.setText("" + turn_count);
	  players_element.addContent(taken_turn_element);
	  
	  org.jdom2.Element rounds_element = new org.jdom2.Element("rounds");
	  rounds_element.setText("" + (game_round - 1));
	  players_element.addContent(rounds_element);

	  org.jdom2.Element order_element = new org.jdom2.Element("order");
	  for (BrassPlayer brass_player : turn_order)
	  {
		  org.jdom2.Element id_element = new org.jdom2.Element("id");
		  id_element.setText("" + brass_player.getPlayerID());
		  order_element.addContent(id_element);
	  }
	  players_element.addContent(order_element);
	  
	  for (BrassPlayer brass_player : brass_players)
	  {
		  org.jdom2.Element player_element = brass_player.writeXML();
		  players_element.addContent(player_element);
	  }
	  
      return players_element;
   }
   
	void readXML(org.jdom2.Element players_element, boolean brass_phase, BrassDeck brass_deck, BrassTrack brass_track)
	{		
		org.jdom2.Element active_element = players_element.getChild("active");
		active_player_id = Integer.parseInt(active_element.getText());

		org.jdom2.Element taken_element = players_element.getChild("turn");
		turn_count = Integer.parseInt(taken_element.getText());
	  
		org.jdom2.Element rounds_element = players_element.getChild("rounds");
		game_round = Integer.parseInt(rounds_element.getText()) + 1;
		is_first_turn = true;
		if (game_round > 1 || brass_phase) is_first_turn = false;
		
		turn_order.clear();
		org.jdom2.Element order_element = players_element.getChild("order");
		List<org.jdom2.Element> order_list_element = order_element.getChildren("id");
		
		for (int i = 1; i <= 4; i++)
		{
			org.jdom2.Element id_element = order_list_element.get(i - 1);
			int turn_id = Integer.parseInt(id_element.getText());
			BrassPlayer brass_player = brass_players.get(turn_id - 1);
			turn_order.add(brass_player);
			brass_player.setTurnOrderImageLoc(turn_order_locations.get(i-1).getX(),turn_order_locations.get(i-1).getY());
		}
		active_player_id = turn_order.get(turn_count - 1).getPlayerID();
		
		List<org.jdom2.Element> players_list_element = players_element.getChildren("player");
	  
		for (org.jdom2.Element player_element : players_list_element)
		{
			org.jdom2.Element id_element = player_element.getChild("id");
			int player_id = Integer.parseInt(id_element.getText());
			
			BrassPlayer brass_player = brass_players.get(player_id - 1);
			brass_player.readXML(player_element, brass_deck, brass_track);
		}
