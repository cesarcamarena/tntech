...
class BrassDeck
{
	...
	BrassCard getUnshuffledCard(int card_id)
	{
		return brass_deck.get(card_id - 1);
	}
	
	org.jdom2.Element writeXML()
   {
      org.jdom2.Element shuffled_deck_element = new org.jdom2.Element("deck");
	  
	  for (BrassCard brass_card : shuffled_deck)
	  {
		  org.jdom2.Element card_element = brass_card.writeXML();
		  shuffled_deck_element.addContent(card_element);
	  }

      return shuffled_deck_element;
   }
   
	void readXML(org.jdom2.Element deck_element)
	{	
		shuffled_deck.clear();
		List<org.jdom2.Element> cards_element = deck_element.getChildren("card");
		...
		for (org.jdom2.Element card_element : cards_element)
		{
			...
			shuffled_deck.add(brass_deck.get(card_id - 1));
		}
	}
}
